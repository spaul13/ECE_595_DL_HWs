...
dls = DLStudio(
                  ...
                  image_size = [32,32],
                  path_saved_model = "C:\\temp\\hw6_model_org",
                  momentum = 0.9,
                  learning_rate = 1e-5,
                  epochs = 2,
                  batch_size = 4,
...
class myPurdueShapes5Dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        #skimage.filters.gaussian(image, sigma=1, output=None, mode='nearest', cval=0, multichannel=True, preserve_range=False, truncate=4.0)
        #self.dataset[idx] = skimage.filters.gaussian(self.dataset[idx], sigma=1,mode='nearest',multichannel=True)
        #print("\n current noise levele : %f" %estimate_sigma(self.dataset[idx],average_sigmas=True))
        r = np.array( self.dataset[idx][0] )
        g = np.array( self.dataset[idx][1] )
        b = np.array( self.dataset[idx][2] )
        """
        r = skimage.filters.gaussian(r, sigma=2,mode='nearest',multichannel=None)
        g = skimage.filters.gaussian(g, sigma=2,mode='nearest',multichannel=None)
        b = skimage.filters.gaussian(b, sigma=2,mode='nearest',multichannel=None)
        """
        R,G,B = r.reshape(32,32), g.reshape(32,32), b.reshape(32,32)
        """
        #Noise-level estimation
        noise_R = estimate_sigma(R,average_sigmas=True)
        noise_G = estimate_sigma(G,average_sigmas=True)
        noise_B = estimate_sigma(B,average_sigmas=True)
        #print(noise_R,noise_G,noise_B)
        #self.noise_level.append((noise_R+noise_G+noise_B)/3)
        #print("\n max=%f, min=%f" %(max(self.noise_level), min(self.noise_level)))
        """
        #"""
        #smoothing 
        #"""
...
#                    if self.dl_studio.debug_train and ((epoch==0 and (i==0 or i==9 or i==99)) or i%500==499):
                ...
                ' '.join('%10s' % dataserver_train.class_labels[labels[j].item()] for j in range(dls.batch_size)))
            """
...
#                  if self.dl_studio.debug_train and ((epoch==0 and (i==0 or i==9 or i==99)) or i%500==499):
                ...
                print("[epoch=%d iter=%d:] Predicted Labels: " % (epoch+1, i+1) + 
                 ' '.join('%10s' % dataserver_train.class_labels[predicted[j].item()] 
...
def noiselevel_training(net, epochs, train_dataloader_1, train_dataloader_2, train_dataloader_3,train_dataloader_4):        
    ...
    """
    new_label = []
    for count in range(dls.batch_size):
        new_label.append(float(noise_level))
    label_arr = torch.from_numpy(np.asarray(new_label))
    """
    print(len(train_dataloader_1))
...
def custom_noise_testing(net, test_dataloader, path):
    ...
    print("\n the correct noise label for overall dataset %d" %max_index)
    return max_index 
...
def main():
    ...
    """
    model1 = detector.LOADnet2(skip_connections=True, depth=32)
    model2 = detector.LOADnet2(skip_connections=True, depth=64)
    model3 = detector.LOADnet2(skip_connections=True, depth=128)
    model4 = detector.LOADnet2(skip_connections=True, depth=256)
    """
    ...
    for i in range(len(noise_level_list)):
        print("\n =========== \n Noise level = %d \n ============ \n"%noise_level_list[i])
        dataserver_train = myPurdueShapes5Dataset(train_or_test = 'train',dl_studio = dls, dataset_file = dataset_train_list[i])
        dataserver_test = myPurdueShapes5Dataset( train_or_test = 'test',dl_studio = dls,dataset_file = dataset_test_list[i])
        train_dataloader[i], test_dataloader[i] = custom_load_PurdueShapes5_dataset(dataserver_train, dataserver_test)
    """
    #total_dataloader = MyLoader([train_dataloader_1,train_dataloader_2, train_dataloader_3, train_dataloader_4])
    print("\n ... commenting out the noise level prediction ... \n")
    """
    ...
    max_index_list = []
    print("\n testing on testdataloader_1 \n")
    max_index = custom_noise_testing(noise_model,test_dataloader_1, path)
    max_index_list.append(max_index)
    max_index = custom_noise_testing(noise_model,test_dataloader_2, path)
    max_index_list.append(max_index)
    max_index = custom_noise_testing(noise_model,test_dataloader_3, path)
    max_index_list.append(max_index)
    max_index = custom_noise_testing(noise_model,test_dataloader_4, path)
    max_index_list.append(max_index)
    #training for models for tackling 4 noise labels
    print("\n ... training for depth 32 .... \n")
    model1 = custom_run_code_for_training_with_CrossEntropy_and_MSE_Losses(model1, train_dataloader_1)
    print("\n ... training for depth 64 .... \n")
    model2 = custom_run_code_for_training_with_CrossEntropy_and_MSE_Losses(model2, train_dataloader_2)
    print("\n ... training for depth 128 .... \n")
    model3 = custom_run_code_for_training_with_CrossEntropy_and_MSE_Losses(model3, train_dataloader_3)
    print("\n ... training for depth 256 .... \n")
    ...
    for i in range(len(max_index_list)):
        ...
        print("\n testing on testdataloader_%s \n" %str(i+1))
        print("\n predicted noise level %d percent and actual = %d \n" %(noise_level_list[max_index], i+1))
        if(max_index == 0):
    ...
    #custom_run_code_for_training_with_CrossEntropy_and_MSE_Losses(model, train_dataloader, dataserver_train)
    #print(max(noise_level), min(noise_level))
