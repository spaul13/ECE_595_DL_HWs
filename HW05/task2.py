...
#Task 2: skimage.filters.gaussian() a gaussian smoothing function is applied to each R,G,B channels 
#after resizing the images to 32X32. After smoothing accuracy doesn't improve at all. For instance
#the classification accuracy for 50% noise-level dataset before and after smoothing is 61%.
#For dataset with 20% and 80% noise level smoothing reduces the accuracy a little bit (2%-3%).
#Experimentally observed: higher sigma value in gaussian smoothing function degrades the accuracy 
#down to 20% from 68%.
...
class myPurdueShapes5Dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        """
        #smoothing 
        R = skimage.filters.gaussian(R, sigma=1,mode='nearest',multichannel=None)
        G = skimage.filters.gaussian(G, sigma=1,mode='nearest',multichannel=None)
        B = skimage.filters.gaussian(B, sigma=1,mode='nearest',multichannel=None)
