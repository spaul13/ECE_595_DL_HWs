...
#Task 3: This includes a noise level classification module. For each image noise level added as the 
#6th parameter while loading or saving the dataset to a .pt file. The small CNN classification 
#module has 8.2M parameters where it can classify the 4 datasets to its respective noise level with 
#100% accuracy while per image average testing accuracy~0.91. I tried three different classification 
# models and V3 provides the overall best accuracy while the previous 2 models seem to be biased to
#some train datasets much more than others. After the noise detection, each test_loader 
#corresponding to different noise level sent to  LOADnet2 with same depth. In this task, noise level
#detection result is not used intelligently to assign different LOADnet2 models for different noise
#levels. 
#here the classification accuracy for the datasets doesn't improve as we are not adaptively using
#the noise level information.   
...
def main():
    ...
    max_index_list, confusion_matrix = [], []
    print("\n testing on testdataloader_1 \n")
    max_index, conf_1 = custom_noise_testing(noise_model,test_dataloader_1, path)
    max_index_list.append(max_index)
    confusion_matrix.append(conf_1)
    max_index, conf_2 = custom_noise_testing(noise_model,test_dataloader_2, path)
    max_index_list.append(max_index)
    confusion_matrix.append(conf_2)    
    max_index, conf_3 = custom_noise_testing(noise_model,test_dataloader_3, path)
    max_index_list.append(max_index)
    confusion_matrix.append(conf_3)
    max_index, conf_4 = custom_noise_testing(noise_model,test_dataloader_4, path)
    max_index_list.append(max_index)
    confusion_matrix.append(conf_4)
